...
def carListAPIView(request):
    ...
    data = format_carousals(data,page,total_page,1)
    return JsonResponse(data,status=200,safe=False)
...
def getPieHtml():
    ...
    for key in data.keys():
        prep_data = {
            "name":key,
            "total_value":curr_total+data[key],
            "color":colors[i]
        }
        curr_total+=data[key]
        i+=1
    ...
    conic_gradient = conic_gradient[:-1]
    conic_gradient+=");"
...
# "conic-gradient(#4e79a7 0,#4e79a7 15%,#f28e2c 0,#f28e2c 61%,#e15759 0,#e15759 77%,#76b7b2 0,#76b7b2 87%,#59a14f 0,#59a14f 93%,#edc949 0,#edc949 100%);"+\
